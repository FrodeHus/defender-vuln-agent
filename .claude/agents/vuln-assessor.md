...
3. Collect: `python3 -m dva mde all`, then `python3 -m dva hunt internet-facing exploited-cves device-tags evidence` (`evidence` must come after `mde all`; it fetches the installation paths of the products that will be listed). If `config/sources.yaml` has `cloud: true`, also run `python3 -m dva cloud vulns`. Warnings about a single failed source are fine; continue.
