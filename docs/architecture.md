...
`exploit_term` is `intel.exploit_maturity` when the CVE server set it (parsed from `triage_cve`'s `PoC:`/`KEV:` lines, `check_poc_exists`'s `Confidence:` label, or `check_exploit_availability`'s public PoC count — public/weaponized/high maps to 1.0, medium to 0.85, low/poc to 0.7, five or more public sources raises the floor to 0.85), otherwise Defender's exploitability level (0, 0.5, 0.75, 1.0 for none, public, verified, in kit). `[ransomware]` is 1 when the CVE server marks known ransomware use (triage `KEV:` line or `check_kev`'s `Ransomware Use: Known`). KEV and ransomware do not depend on triage: `dva enrich --fetch` (or `dva kev`) downloads CISA's Known Exploited Vulnerabilities catalogue once a day into `<cache>/kev.json` and `dva score` marks every CVE in the estate that it lists (`dva/kev.py`), so a KEV entry is never missed because the CVE was not among the ones triaged. Without enrichment, EPSS is 0, KEV comes from the catalogue alone, and `exploit_term` falls back to Defender's level. The five weights sum to 1.10 by default, so `threat` is capped implicitly by the later `min(1, …)` on the product score.

Per asset: `multiplier = min(asset_cap, 1 + sum of applicable bonuses)`, where bonuses also include `attack_path` (the device sits on a Defender for Cloud attack path, cloud tenants only) and `mitigated` (a negative bonus/discount when all of the tenant's chosen compensating controls are compliant on the device).
...
Products are ordered by their preliminary score, computed with what is already known (cached intel, stale included, plus the KEV catalogue). Every product inside the report window (`top_n` + 15) and every product at or above `enrich_threshold` gets its top `enrich_top_per_product` CVEs by threat score (KEV and cached EPSS first, then Defender CVSS, exploitability and recency) selected, skipping ids already fresh in the cache, until `enrich_max_cves` is reached. `dva enrich --fetch` starts the CVE server as a subprocess and calls `triage_cve` once per id, merges the results, and only then selects the CVEs to describe: the driving CVEs of each product ranked with the intel now in the cache, so `lookup_cve` and `get_vendor_advisory` land on the CVEs the report will actually show (the server is started once per phase). The parser reads the server's text output. On a mid-size estate this is a few dozen calls per run, none of which pass through the agent's context. An entry older than `cache_ttl_days` is triaged again for fresh EPSS/KEV/PoC signals, but its description, vector, CWE and vendor advisories are carried over from the expired entry (`select_describe` trusts a stale description), so the two description calls happen once per CVE.
