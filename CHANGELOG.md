...
- EPSS is rounded to three decimals in `findings.json`, and CVE titles derived from descriptions are cut at a word boundary with an ellipsis.
- A `DVA_TENANT_DIR` exported from the shell is honoured: the lone configured tenant is no longer auto-activated over it, so fixture runs and scripts outside pytest cannot land in a real tenant's runs and caches.
- Docs: how to run the agent on a local model through Ollama, with `ollama launch claude` as the primary path and the manual endpoint variables as the fallback, and why Claude Code must start in bare mode without plugins or MCP servers for the request to fit a local context window (`docs/install.md`, with pointers from the README, `docs/usage.md`, `docs/troubleshooting.md` and `CLAUDE.md`).
