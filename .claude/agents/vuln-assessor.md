...
4. Enrich: run `python3 -m dva enrich --list`. For each printed line `{"chunk": n, "cve_ids": [...]}` call `bulk_cve_lookup` with `cve_ids` set to that list, save the tool result unchanged to `$DVA_RUN/cve-chunk-<n>.json` with a quoted heredoc: `cat <<'JSON' > "$DVA_RUN/cve-chunk-<n>.json"` ... `JSON`, and run `python3 -m dva enrich --store $DVA_RUN/cve-chunk-<n>.json`. The delimiter must be quoted (`<<'JSON'`, not `<<JSON`) so the shell writes the JSON byte for byte instead of expanding `$`, backticks and backslash escapes inside it. Then for every CVE the bulk result marks as in KEV or with EPSS ≥ 0.5, call `triage_cve` with `cve_id` and `depth` = `standard`, save to `$DVA_RUN/cve-triage-<id>.json` the same way with `cat <<'JSON' > "$DVA_RUN/cve-triage-<id>.json"` ... `JSON`, and store it the same way. If the CVE server is unreachable, say so and continue; scoring works without it.
