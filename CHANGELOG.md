...
- Docs: how to run the agent on a local model through Ollama, with `ollama launch claude` as the primary path and the manual endpoint variables as the fallback, and why Claude Code must start in bare mode without plugins or MCP servers for the request to fit a local context window (`docs/install.md`).
